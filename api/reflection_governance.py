...
import logging
import uuid
from api.deps import get_db, get_llm_client, get_skill_governance, verify_token
...
async def get_pending_rules(db=Depends(get_db)):
    ...
    try:
        ...
        return ok({"data": [dict(zip(columns, row)) for row in rows]})
...
async def approve_rule(rule_id: str, db=Depends(get_db)):
    ...
    try:
        ...
        if bool(row[0]):
            return ok({"status": "already_active", "message": f"Rule {rule_id} 已处于激活状态。", "rule_id": rule_id})
        cursor = await db.conn.execute(
    ...
    return ok({"status": "success", "message": f"Rule {rule_id} activated.", "rule_id": rule_id})
...
async def author_review(
    ...
    try:
        ...
        return ok({"status": "reviewed", "review": result, "prompt": prompt})
    except Exception as exc:
...
async def submit_skill_candidate(payload: SkillCandidateRequest, governance=Depends(get_skill_governance)):
    ...
    try:
        ...
        status = governance.auto_review(candidate_id)
        return ok({"status": status, "candidate_id": candidate_id})
    except SkillGovernanceError as exc:
        raise HTTPException(status_code=409, detail=str(exc))


@router.post("/reflection/skills/candidates/{candidate_id}/approve")
async def approve_skill_candidate(candidate_id: str, payload: SkillReviewRequest, governance=Depends(get_skill_governance)):
    """作者/管理员人工批准候选；批准后仍需显式 promote 才能版本化。"""
    try:
        status = governance.manual_approve(
...
async def reject_skill_candidate(candidate_id: str, payload: SkillReviewRequest, governance=Depends(get_skill_governance)):
    """人工驳回候选并保留审核审计记录。"""
    try:
        status = governance.reject(
...
async def list_skill_candidates(status: str | None = None, governance=Depends(get_skill_governance)):
    """候选清单(可按状态过滤:PENDING/MANUAL_APPROVED/GRAYSCALE/FULL/REJECTED)。"""
    return ok({"candidates": governance.list_candidates(status=status)})


@router.post("/reflection/skills/candidates/{candidate_id}/promote")
async def promote_skill_candidate(candidate_id: str, governance=Depends(get_skill_governance)):
    """人工批准后版本化晋升。Batch 4：唯一入口 = promote_through_gate——
    必须同时满足「作者人工批准 MANUAL_APPROVED」与「已过测试」才落地；
    无批准/无测试/基线缺失/版本漂移一律拒绝（fail-closed），绝不绕过门直写 active。"""
    try:
        version = governance.promote_through_gate(candidate_id, operator="author")
        return ok({"candidate_id": candidate_id, "version": version})
    except SkillGovernanceError as exc:
        raise HTTPException(status_code=409, detail=str(exc))


@router.post("/reflection/skills/candidates/{candidate_id}/grayscale")
async def start_skill_grayscale(candidate_id: str, payload: SkillGrayscaleRequest, governance=Depends(get_skill_governance)):
    """灰度发布:按比例在部分任务上生效(P2灰度消费的治理侧)。"""
    try:
        governance.start_grayscale(candidate_id, percent=payload.percent)
        return ok({"candidate_id": candidate_id, "percent": payload.percent, "status": "GRAYSCALE"})
    except SkillGovernanceError as exc:
        raise HTTPException(status_code=409, detail=str(exc))


@router.post("/reflection/skills/candidates/{candidate_id}/full")
async def promote_skill_full(candidate_id: str, governance=Depends(get_skill_governance)):
    """灰度验证通过,全量转正。"""
    try:
        governance.promote_full(candidate_id)
        return ok({"candidate_id": candidate_id, "status": "FULL"})
    except SkillGovernanceError as exc:
        raise HTTPException(status_code=409, detail=str(exc))


@router.post("/reflection/skills/candidates/{candidate_id}/stop-gray")
async def stop_skill_grayscale(candidate_id: str, governance=Depends(get_skill_governance)):
    """中止灰度(回退至未放量状态)。"""
    try:
        governance.stop_grayscale(candidate_id)
        return ok({"candidate_id": candidate_id, "status": "STOPPED"})
    except SkillGovernanceError as exc:
        raise HTTPException(status_code=409, detail=str(exc))


@router.post("/reflection/skills/candidates/{candidate_id}/rollback")
async def rollback_skill_candidate(candidate_id: str, payload: SkillRollbackRequest, governance=Depends(get_skill_governance)):
    """版本回滚至指定历史版本。"""
    try:
        version = governance.rollback(
